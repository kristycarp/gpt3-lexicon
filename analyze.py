...
d = "outputs/alprazolam"
fs = os.listdir(d)
fs = [f for f in fs if os.path.isfile(os.path.join(d, f))]

outfname = "alprazolam_out.csv"
dic = dict()
dic["model"] = []
dic["temp"] = []
dic["freq"] = []
...
for f in tqdm(fs):
    ...
    df = df.loc[df["seed for prompt"] == seed]
    ...
    tmp = df.loc[df["Seed of GPT-3 term in RedMed"] == seed]
    ...
    not_redmed = df.loc[df["Seed of GPT-3 term in RedMed"] != seed]
    ...
    # what other redmed seeds appear and how many times each?
    # what terms appear that are not in redmed?
    # what terms appear that are not in redmed and not validated by google?
